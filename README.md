...
- Копіювання паролю в буфер обміну з авто-очищенням через `PM_CLIPBOARD_CLEAR_SECONDS` секунд (за замовч. 15). Підтримує `xclip` / `xsel` / `wl-copy` (Linux), `pbcopy` (macOS), `clip` / `clip.exe` (Windows / WSL).
